            ...
            return (
              ...
              >
                ...
                <p className="mb-6 text-base leading-relaxed text-justify text-gray-600">
                  {t(item.description).length > 150 ? t(item.description).substring(0, 150) + '...' : t(item.description)}
                </p>
                {/* Read More Button - now opens modal and is aligned right */}
                <div className="w-full text-right">
                  <button
                    onClick={() => openModal(t(item.title) || item.fallbackTitle, t(item.description))}
            ...
            <div className="text-center transition-all duration-500 ease-in-out opacity-0 animate-[fadeIn_0.5s_ease-in-out_forwards]">
              ...
              <div className="flex flex-col items-center justify-center space-y-3 sm:flex-row sm:space-y-0 sm:space-x-3">
                <div className="flex items-center justify-center w-12 h-12 text-base font-bold bg-white rounded-full shadow-sm text-primary">
                  {t(testimonials[activeTestimonial].initials)}
                </div>
                <div className="text-center sm:text-left">
                  <div className="text-base font-semibold text-white">{t(testimonials[activeTestimonial].author)}</div>
                  <div className="text-sm text-gray-600">{t(testimonials[activeTestimonial].title)}</div>
                  <div className="text-xs text-gray-600">{t(testimonials[activeTestimonial].country)}</div>
                </div>
              </div>
            </div>
