...
export default function UpcomingMaintenance() {
  ...
  const fetchMaintenanceData = async () => {
    try {
      ...
      setNotifications(data.maintenanceNotifications || []);
      
      // Extract service names from upcoming features
      const serviceNames = (data.upcomingFeatures || []).map(f => f.service);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchMaintenanceData, 5 * 60 * 1000);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-400">Loading release notifications...</p>
        </div>
      </div>
    );
  }
