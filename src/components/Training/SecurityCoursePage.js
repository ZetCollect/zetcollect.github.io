import React, { useState, useEffect } from "react";
import { Link } from "react-router-dom";
import coursesData from "../../data/coursesData.json";
import Footer from '../../components/Footer';
import { useLanguage } from '../../context/LanguageContext';

const SecurityCoursePage = () => {
  const { t } = useLanguage();
  const [filter, setFilter] = useState("All");
  const [lessons, setLessons] = useState([]);
  const [activeTab] = useState("ZetCollect Security");

  useEffect(() => {
    const securityCourse = coursesData.find(
      (course) => course.id === "security"
    );
    if (securityCourse) {
      setLessons(securityCourse.lessons);
    }
  }, []);

  const filteredLessons = lessons.filter((lesson) => {
    if (filter === "All") return true;
    return lesson.status === filter;
  });

  return (
    <>
      {/* Header Section with Navigation and Filter */}
      <div className="px-4 py-12 bg-secondary sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="pl-4 sm:pl-6 lg:pl-8">
            <h1 className="mb-6 text-4xl font-extrabold text-gray-900 lg:text-5xl">
              {t("security.title")}
            </h1>

            {/* Navigation Tabs and Filter */}
            <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0 md:space-x-6">
              <nav className="flex flex-wrap text-lg font-medium gap-x-6 gap-y-2">
                <Link
                  to="/training-and-certification"
                  className={`pb-1 border-b-2 ${
                    activeTab === "All Courses"
                      ? "border-primary text-primary"
                      : "border-transparent text-gray-700 hover:text-primary"
                  }`}
                >
                  {t("security.allCourses")}
                </Link>
                <Link
                  to="/training/product-knowledge"
                  className={`pb-1 border-b-2 ${
                    activeTab === "ZetCollect Product Knowledge"
                      ? "border-primary text-primary"
                      : "border-transparent text-gray-700 hover:text-primary"
                  }`}
                >
                  {t("security.productKnowledge")}
                </Link>
                <Link
                  to="/training/onboarding"
                  className={`pb-1 border-b-2 ${
                    activeTab === "Onboarding and Issue Resolution"
                      ? "border-primary text-primary"
                      : "border-transparent text-gray-700 hover:text-primary"
                  }`}
                >
                  {t("security.onboardingAndIssueResolution")}
                </Link>
                <Link
                  to="/training/security"
                  className={`pb-1 border-b-2 ${
                    activeTab === "ZetCollect Security"
                      ? "border-primary text-primary"
                      : "border-transparent text-gray-700 hover:text-primary"
                  }`}
                >
                  {t("security.security")}
                </Link>
              </nav>

              {/* Filter Dropdown */}
              <div className="relative w-full md:w-auto">
                <select
                  className="w-full py-2 pl-4 pr-8 border border-gray-300 rounded-md appearance-none focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
                  value={filter}
                  onChange={(e) => setFilter(e.target.value)}
                >
                  <option value="All">{t("security.filter.all")}</option>
                  <option value="Completed">
                    {t("security.filter.completed")}
                  </option>
                  <option value="In Progress">
                    {t("security.filter.inProgress")}
                  </option>
                </select>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* Lessons Grid */}
      <div className="px-4 py-16 mx-auto lg:py-24 max-w-7xl">
        {filteredLessons.length > 0 ? (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredLessons.map((lesson) => (
              <Link
                to={`/course-lesson/${lesson.id}`}
                key={lesson.id}
                className="block overflow-hidden transition-transform duration-300 bg-white rounded-lg shadow-md hover:scale-105 hover:shadow-xl"
              >
                <img
                  src={lesson.image}
                  alt={t(lesson.titleKey)}
                  className="object-cover w-full h-48"
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src =
                    "/No-Image-Placeholder.png";
                  }}
                />
                <div className="p-6">
                  <h3 className="mb-2 text-xl font-bold text-gray-900 transition-colors group-hover:text-primary">
                    {t(lesson.titleKey)}
                  </h3>
                  <p className="text-sm text-gray-600 line-clamp-3">
                    {t(lesson.descriptionKey)}
                  </p>
                </div>
              </Link>
            ))}
          </div>
        ) : (
          <div className="text-lg text-center text-gray-600">
            {t("security.noLessons")}
          </div>
        )}
      </div>
      <Footer />
    </>
  );
};

export default SecurityCoursePage;